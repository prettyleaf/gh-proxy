...
В шапке — версия, с которой запущен процесс; по клику раскрывается сборка за ней:
ветка, коммит, время сборки, номер прогона CI. Рядом счётчик звёзд репозитория и
кнопка обновления, когда ждать пять секунд до следующего опроса долго. Версия
становится жёлтой и слегка пульсирует, если вышел релиз новее. Эти два числа —
единственное на странице, что берётся снаружи, и запрашивает их *браузер* прямо
у api.github.com с получасовым кэшем: сам прокси никуда не ходит, а тот, у кого
нет доступа к GitHub, получает страницу целиком, только без счётчиков.
