...
Страница статуса (`GHP_STATUS_PATH`) — единственное исключение из «наружу
торчит только префикс», и поэтому она выключена по умолчанию. Что важно про неё
знать:

* пока переменная не задана, никакого пути нет — тот же `404`, что и на всё
  остальное. Неверный токен и неизвестный подпуть под ней отвечают так же;
* токена на странице нет и в JSON он не попадает. Есть: счётчики, причины
  отказов, `owner/repo` последних запросов, префикс монтирования и остальная
  конфигурация. Для чужого это подсказка, что здесь за сервис, — отсюда и
  требование её закрыть;
* `GHP_STATUS_AUTH=none` перекладывает проверку на reverse-proxy целиком.
  Если `location` окажется без `auth_request`/`auth_basic`, страница публична.
  Конфиг с tinyauth и способ это проверить — в [nginx.md](nginx.md#страница-статуса);
* `?token=...` (форма для браузера при `GHP_STATUS_AUTH=token`) — это токен в
  URL: он осядет в `access_log` nginx, если тот не выключен для этого блока, и в
  истории браузера;
* запросы к самой странице в счётчики не попадают, так что по ним нельзя понять,
  кто и когда её открывал, — как и для остального трафика.

Метрики без публичного пути вообще: админский listener (`GHP_ADMIN_LISTEN`,
loopback) отдаёт `/status`, `/status/json` и `/metrics` всегда.
...
| `?token=` у страницы статуса | попадёт в `access_log`, если он включён для этого `location`; страницу лучше закрывать чужой аутентификацией (`GHP_STATUS_AUTH=none`) |
